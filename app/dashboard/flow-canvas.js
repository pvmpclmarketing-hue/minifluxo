...
const blocks = {
  ...
  messageAgent: { label:'Agente de mensagens', icon:'3x', tone:'purple', description:'Cria e envia 3 mensagens personalizadas', config:{ model:'gpt-5-mini', saveTo:'mensagens_personalizadas', intro:'Aqui estão as 3 versões das suas mensagens. Escolha a que mais combina com você e copie para enviar junto da música:' } },
  pixCopyPaste: { label:'Gerar PIX copia e cola', icon:'PIX', tone:'emerald', description:'Gera uma cobrança Efi e envia o código Pix', config:{ amount:'49.90', expiration:3600, description:'Pagamento do pedido', saveTo:'pix' } },
...
function summary(data) {
  ...
  if(data.kind==='messageAgent') return 'Envia 3 versões: emocional, natural e surpresa';
  if(data.kind==='pixCopyPaste') return `PIX de R$ ${c.amount || '0,00'} | copia e cola`;
...
function ConfigModal({ node, credentials, onClose, onSave }) {
  ...
  return <div className={styles.backdrop} onMouseDown={onClose}><section className={styles.modal} onMouseDown={e=>e.stopPropagation()}><header><span className={styles[node.data.tone]}>{node.data.icon}</span><div><h3>Editar {node.data.title}</h3><p>Configure esta etapa do fluxo.</p></div><button onClick={onClose}>X</button></header><div className={styles.modalBody}>
    ...
    {kind==='messageAgent' && <><p className={styles.note}>Usa automaticamente o nome, a história, a relação e o contexto que chegaram do site. O agente cria três versões e envia cada uma em uma mensagem separada no WhatsApp, facilitando a cópia.</p><Field label={`Chave API GPT ${credentials.gptConfigured ? '(ja cadastrada - digite somente para trocar)' : ''}`}><input type="password" value={apiKey} onChange={e=>setApiKey(e.target.value)} placeholder={credentials.gptConfigured?'Nova chave opcional':'sk-...'} autoComplete="new-password"/></Field><Field label="Mensagem antes das 3 versões"><textarea value={config.intro||''} onChange={e=>set('intro',e.target.value)} placeholder="Aqui estão as 3 versões das suas mensagens..."/></Field><p className={styles.note}>O prompt é fixo. As versões também ficam salvas em <code>{'{flow_data.mensagens_personalizadas.emocional}'}</code>, <code>{'{flow_data.mensagens_personalizadas.natural}'}</code> e <code>{'{flow_data.mensagens_personalizadas.surpresa}'}</code>.</p><Field label="Campo-base para salvar as 3 mensagens"><input value={config.saveTo||''} onChange={e=>set('saveTo',e.target.value)} placeholder="mensagens_personalizadas"/></Field></>}
    {kind==='pixCopyPaste' && <><p className={styles.note}>Cria uma cobrança Pix dinâmica na Efi e envia somente o código copia e cola ao cliente. Configure suas credenciais, certificado P12 e chave Pix na aba <b>APIs</b>.</p><Field label="Valor da cobrança (R$)"><input inputMode="decimal" value={config.amount||''} onChange={e=>set('amount',e.target.value)} placeholder="49.90"/></Field><Field label="Expira em (segundos)"><input type="number" min="1" value={config.expiration||3600} onChange={e=>set('expiration',Number(e.target.value))}/></Field><Field label="Descrição para o pagador"><textarea value={config.description||''} onChange={e=>set('description',e.target.value)} placeholder="Pagamento do pedido"/></Field><Field label="Campo para salvar a cobrança"><input value={config.saveTo||'pix'} onChange={e=>set('saveTo',e.target.value)} placeholder="pix"/></Field><p className={styles.note}>O código fica disponível para os próximos blocos em <code>{'{flow_data.pix.copia_e_cola}'}</code>. Cada execução cria uma cobrança própria e não compartilha dados entre contas.</p></>}
