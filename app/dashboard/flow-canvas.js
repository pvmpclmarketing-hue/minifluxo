...
const blocks = {
  ...
  wait: { label:'Aguardar / Delay', icon:'W', tone:'orange', description:'Espera resposta ou segue apos um tempo', config:{ mode:'response', duration:1, unit:'minutos', field:'comprovante', preMessage:'' } },
...
function summary(data) {
  ...
  if(data.kind==='wait') return c.mode==='delay'?`Segue apos ${c.duration || 1} ${c.unit || 'minutos'}`:`Espera resposta | salva: ${c.field || '-'}`;
...
function ConfigModal({ node, credentials, onClose, onSave }) {
  ...
  return <div className={styles.backdrop} onMouseDown={onClose}><section className={styles.modal} onMouseDown={e=>e.stopPropagation()}><header><span className={styles[node.data.tone]}>{node.data.icon}</span><div><h3>Editar {node.data.title}</h3><p>Configure esta etapa do fluxo.</p></div><button onClick={onClose}>X</button></header><div className={styles.modalBody}>
    ...
    {kind==='wait' && <><Field label="Como esta etapa deve continuar"><select value={config.mode||'response'} onChange={e=>set('mode',e.target.value)}><option value="response">Quando o cliente responder</option><option value="delay">Automaticamente apos um delay</option></select></Field>{config.mode==='delay'?<><p className={styles.note}>O fluxo prossegue sozinho depois do período escolhido. A execução pode ocorrer com até cerca de 1 minuto adicional.</p><div className={styles.two}><Field label="Tempo"><input type="number" min="1" value={config.duration||1} onChange={e=>set('duration',Number(e.target.value))}/></Field><Field label="Unidade"><select value={config.unit||'minutos'} onChange={e=>set('unit',e.target.value)}><option value="minutos">Minutos</option><option value="horas">Horas</option><option value="dias">Dias</option></select></Field></div></>:<><Field label="Campo para salvar a resposta"><input value={config.field||''} onChange={e=>set('field',e.target.value)} placeholder="comprovante"/></Field><Field label="Mensagem antes de aguardar"><textarea value={config.preMessage||''} onChange={e=>set('preMessage',e.target.value)}/></Field></>}</>}
