...
const blocks = {
  ...
  messageAgent: { label:'Agente de mensagens', icon:'3x', tone:'purple', description:'Cria 3 mensagens personalizadas para a música', config:{ model:'gpt-5-mini', saveTo:'mensagens_personalizadas' } },
...
function summary(data) {
  ...
  if(data.kind==='messageAgent') return 'Gera 3 versões: emocional, natural e surpresa';
...
function ConfigModal({ node, credentials, onClose, onSave }) {
  ...
  return <div className={styles.backdrop} onMouseDown={onClose}><section className={styles.modal} onMouseDown={e=>e.stopPropagation()}><header><span className={styles[node.data.tone]}>{node.data.icon}</span><div><h3>Editar {node.data.title}</h3><p>Configure esta etapa do fluxo.</p></div><button onClick={onClose}>X</button></header><div className={styles.modalBody}>
    ...
    {kind==='messageAgent' && <><p className={styles.note}>Usa automaticamente o nome, a história, a relação e o contexto que chegaram do site. O agente cria três versões de introdução para a música: emocional, natural e surpresa.</p><Field label={`Chave API GPT ${credentials.gptConfigured ? '(ja cadastrada - digite somente para trocar)' : ''}`}><input type="password" value={apiKey} onChange={e=>setApiKey(e.target.value)} placeholder={credentials.gptConfigured?'Nova chave opcional':'sk-...'} autoComplete="new-password"/></Field><p className={styles.note}>O prompt é fixo. As mensagens não são enviadas automaticamente: use <code>{'{flow_data.mensagens_personalizadas.emocional}'}</code>, <code>{'{flow_data.mensagens_personalizadas.natural}'}</code> ou <code>{'{flow_data.mensagens_personalizadas.surpresa}'}</code> em um bloco de Mensagem.</p><Field label="Campo-base para salvar as 3 mensagens"><input value={config.saveTo||''} onChange={e=>set('saveTo',e.target.value)} placeholder="mensagens_personalizadas"/></Field></>}
