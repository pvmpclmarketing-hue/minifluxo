...
function nestedMenuOption(value,seen=new Set(),depth=0){if(depth>7||value===null||value===undefined||typeof value!=='object'||seen.has(value))return '';seen.add(value);for(const [key,item] of Object.entries(value)){if(typeof item==='string'&&/^menu-option-\d+$/i.test(item.trim()))return item.trim();if((key==='id'||key==='buttonId'||key==='selectedButtonId'||key==='selectedRowId')&&typeof item==='string'&&/^menu-option-\d+$/i.test(item.trim()))return item.trim();const found=nestedMenuOption(item,seen,depth+1);if(found)return found;}return '';}
function messageText(body){const data=body.data||body;const message=data.message||data.data?.message||{};const content=message.message||message;const interactive=content.interactiveResponseMessage?.nativeFlowResponseMessage?.paramsJson;let interactiveId='';try{const params=interactive?JSON.parse(interactive):{};interactiveId=params.id||params.selectedButtonId||params.selectedRowId||'';}catch{}return nestedMenuOption(body)||data.text||data.body||message.text||content.buttonsResponseMessage?.selectedButtonId||content.buttonsResponseMessage?.selectedDisplayText||content.buttonResponseMessage?.selectedButtonId||content.buttonResponseMessage?.selectedDisplayText||content.listResponseMessage?.singleSelectReply?.selectedRowId||content.listResponseMessage?.singleSelectReply?.title||interactiveId||content.conversation||content.extendedTextMessage?.text||content.imageMessage?.caption||content.videoMessage?.caption||'';}
function isInteractiveReply(body){const data=body.data||body;const message=data.message||data.data?.message||{};const content=message.message||message;return Boolean(nestedMenuOption(body)||content.buttonsResponseMessage||content.buttonResponseMessage||content.listResponseMessage?.singleSelectReply||content.interactiveResponseMessage?.nativeFlowResponseMessage);}
