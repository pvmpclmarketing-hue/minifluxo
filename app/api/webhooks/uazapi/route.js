...
export async function POST(request){
  try{
    ...
    if(existing){const context={...(existing.order_context||{}),last_message:text};const {data:lead}=await db.from('leads').update({order_context:context,status:'in_progress',updated_at:new Date().toISOString()}).eq('id',existing.id).select().single();const {data:flow}=await db.from('flows').select('*').eq('id',existing.order_context.flow_execution.flow_id).eq('owner_id',existing.owner_id).maybeSingle();if(flow?.status==='active')await executeFlow({db,flow,lead,connection,resumeAfterId:existing.order_context.flow_execution.wait_node_id});return NextResponse.json({received:true,resumed:true});}
    if(contacts.length)return NextResponse.json({received:true,ignored:true,reason:'contact_already_in_history'});
    const {data:flow}=await db.from('flows').select('*').eq('id',config.conversation_flow_id).eq('owner_id',config.owner_id).maybeSingle();if(!flow||flow.status!=='active')return NextResponse.json({received:true,ignored:true});const {data:lead,error}=await db.from('leads').insert({owner_id:config.owner_id,name:'Cliente WhatsApp',phone,source:'inbound',music_request:text,status:'in_progress',provider:'uazapi',connection_id:connection.id,order_context:{last_message:text,chat_history:[{role:'user',content:text}]}}).select().single();if(error)throw error;await executeFlow({db,flow,lead,connection});return NextResponse.json({received:true,started:true});
