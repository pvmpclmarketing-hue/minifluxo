...
function messageText(body){const data=body.data||body;const message=data.message||data.data?.message||{};const content=message.message||message;const interactive=content.interactiveResponseMessage?.nativeFlowResponseMessage?.paramsJson;let interactiveId='';try{interactiveId=interactive?JSON.parse(interactive).id||JSON.parse(interactive).selectedButtonId||'':'';}catch{}return data.text||data.body||message.text||content.buttonsResponseMessage?.selectedButtonId||content.buttonsResponseMessage?.selectedDisplayText||content.buttonResponseMessage?.selectedButtonId||content.buttonResponseMessage?.selectedDisplayText||content.listResponseMessage?.singleSelectReply?.selectedRowId||content.listResponseMessage?.singleSelectReply?.title||interactiveId||content.conversation||content.extendedTextMessage?.text||content.imageMessage?.caption||content.videoMessage?.caption||'';}
