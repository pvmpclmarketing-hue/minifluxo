...
export async function POST(request){
  try{
    ...
    const {data:config}=await db.from('connection_flow_configs').select('conversation_flow_id,owner_id').eq('connection_id',connection.id).maybeSingle();if(!config?.conversation_flow_id||config.owner_id!==connection.owner_id)return NextResponse.json({received:true,ignored:true});
    ...
    const {data:flow}=await db.from('flows').select('*').eq('id',config.conversation_flow_id).eq('owner_id',config.owner_id).maybeSingle();if(!flow||flow.status!=='active')return NextResponse.json({received:true,ignored:true});const {data:lead,error}=await db.from('leads').insert({owner_id:config.owner_id,name:name||'Cliente WhatsApp',phone,source:'manual',music_request:text,status:'in_progress',provider:'uazapi',connection_id:connection.id,order_context:{contact_origin:'inbound',last_message:text,chat_history:[{role:'user',content:text}]}}).select().single();if(error)throw error;await executeFlow({db,flow,lead,connection});return NextResponse.json({received:true,started:true});
