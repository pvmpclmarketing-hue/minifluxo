...
export async function POST(request) {
  try {
    ...
    const { data: order, error: orderError } = await db.from('lyric_video_orders').select('id,shotstack_render_id').eq('id', orderId).maybeSingle();
    if (orderError || !order) return NextResponse.json({ error: 'Pedido não encontrado.' }, { status: 404 });
    if (String(order.shotstack_render_id) !== String(renderId)) return NextResponse.json({ error: 'Render não corresponde ao pedido.' }, { status: 403 });
    ...
    if (status === 'done') Object.assign(update, { status: 'complete', output_url: render.url || null, error: null });
    else if (status === 'failed') Object.assign(update, { status: 'failed', error: render.error || render.message || 'A Shotstack não conseguiu renderizar este clipe.' });
