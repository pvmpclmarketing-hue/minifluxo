...
async function deliverAudioTracks(db,lead,connection,audios,intro){
  ...
  const total=Math.min(2,audios.length);for(let index=0;index<total;index+=1){if(sentIndexes.includes(index))continue;const result=await sendAudioConfirmed(connection,currentLead.phone,audios[index],index,total);const messageId=String(result?.messages?.[0]?.id||result?.data?.messages?.[0]?.id||`audio-${index}-${Date.now()}`);messageIds[index]={id:messageId,status:'sent',sent_at:new Date().toISOString()};currentLead=await appendChatMessage(db,currentLead,{id:messageId,direction:'out',type:'audio',text:`🎵 Música ${index+1} de ${total} enviada`});sentIndexes=[...sentIndexes,index];currentLead=await saveDeliveryProgress(db,currentLead,connection,audios,{sent_indexes:sentIndexes,intro_sent:introSent,message_ids:messageIds});if(index<total-1)await wait(Number(process.env.AUDIO_SEND_INTERVAL_MS||2000));}
  return currentLead;
}
