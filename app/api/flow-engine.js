...
async function credentialsFor(db,flowId){const {data}=await db.from('flow_credentials').select('kie_key_cipher,gpt_key_cipher').eq('flow_id',flowId).maybeSingle();return {kie:data?.kie_key_cipher?decryptSecret(data.kie_key_cipher):null,gpt:data?.gpt_key_cipher?decryptSecret(data.gpt_key_cipher):null};}
