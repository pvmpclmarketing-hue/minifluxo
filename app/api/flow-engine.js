...
const PURCHASE_NOTIFICATION_PHONE='5522998951446';
...
async function saveDeliveryProgress(db,lead,connection,audios,progress,status='delivering'){
  const orderContext={...(lead.order_context||{}),delivery:{...(lead.order_context?.delivery||{}),audios,sent_indexes:progress.sent_indexes||[],intro_sent:!!progress.intro_sent,message_ids:progress.message_ids||lead.order_context?.delivery?.message_ids||{}}};
...
async function deliverAudioTracks(db,lead,connection,audios,intro){
  const saved=lead.order_context?.delivery||{};let currentLead=lead;let sentIndexes=Array.isArray(saved.sent_indexes)?saved.sent_indexes:[];let introSent=!!saved.intro_sent;let messageIds={...(saved.message_ids||{})};
  if(intro&&!introSent){await sendText(connection,currentLead.phone,intro);introSent=true;currentLead=await saveDeliveryProgress(db,currentLead,connection,audios,{sent_indexes:sentIndexes,intro_sent:introSent,message_ids:messageIds});}
  const total=Math.min(2,audios.length);for(let index=0;index<total;index+=1){if(sentIndexes.includes(index))continue;const result=await sendAudioConfirmed(connection,currentLead.phone,audios[index],index,total);const messageId=String(result?.messages?.[0]?.id||result?.data?.messages?.[0]?.id||`audio-${index}-${Date.now()}`);messageIds[index]={id:messageId,status:'sent',sent_at:new Date().toISOString()};currentLead=await appendChatMessage(db,currentLead,{id:messageId,direction:'out',type:'audio',url:audios[index],text:`🎵 Música ${index+1} de ${total} enviada`});sentIndexes=[...sentIndexes,index];currentLead=await saveDeliveryProgress(db,currentLead,connection,audios,{sent_indexes:sentIndexes,intro_sent:introSent,message_ids:messageIds});if(index<total-1)await wait(Number(process.env.AUDIO_SEND_INTERVAL_MS||2000));}
  return currentLead;
}
...
export async function executeFlow({db,flow,lead,connection,resumeAfterId=null,resumeHandle=null,audios=[]}){
  ...
  for(let steps=0;node&&steps<50;steps+=1){const kind=node.data?.kind;const config=node.data?.config||{};
    if(kind==='message'){const text=render(config.message,variables);if(text){await sendText(connection,currentLead.phone,text);currentLead=await appendChatMessage(db,currentLead,{direction:'out',type:'text',text});variables=variablesFor(currentLead,{audios:readyAudios});}}
    if(kind==='menu'){const choices=(Array.isArray(config.choices)?config.choices:[]).filter(item=>String(item?.label||'').trim()).slice(0,10);if(!choices.length)throw new Error('Adicione ao menos uma opção ao card Menu.');const text=render(config.message||'Escolha uma opção:',variables);const renderedChoices=choices.map(item=>({...item,label:render(item.label,variables)}));await sendMenu(connection,currentLead.phone,text,renderedChoices);currentLead=await appendChatMessage(db,currentLead,{direction:'out',type:'text',text:`${text}\n\nOpções:\n${renderedChoices.map((item,index)=>`${index+1}. ${item.label}`).join('\n')}`});const context={...(currentLead.order_context||{}),flow_execution:{flow_id:flow.id,menu_node_id:node.id}};await db.from('leads').update({status:'waiting_response',order_context:context,updated_at:new Date().toISOString()}).eq('id',currentLead.id).eq('owner_id',currentLead.owner_id).eq('connection_id',connection.id);return {waiting:true};}
