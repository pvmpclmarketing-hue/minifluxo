...
import { sendAudio, sendMenu, sendText } from './provider';
...
async function runMessageAgent(db,flow,lead,connection,node){
  ...
  const response=await fetch('https://api.openai.com/v1/chat/completions',{method:'POST',headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json'},body:JSON.stringify({model:config.model||'gpt-5-mini',messages:[{role:'system',content:MESSAGE_AGENT_PROMPT},{role:'user',content:`Dados reais do lead (use somente estes dados): ${JSON.stringify(leadContext)}`}]})});
