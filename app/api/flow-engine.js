...
function efiRequest({hostname,path,method='POST',headers={},body='',pfx,passphrase}){return new Promise((resolve,reject)=>{const request=https.request({hostname,path,method,headers:{...headers,'Content-Length':Buffer.byteLength(body)},pfx,passphrase,rejectUnauthorized:true,timeout:20000},response=>{let raw='';response.setEncoding('utf8');response.on('data',chunk=>{raw+=chunk;});response.on('end',()=>{let data={};try{data=JSON.parse(raw);}catch{}resolve({status:response.statusCode||500,data,raw});});});request.on('timeout',()=>request.destroy(new Error('A Efi demorou demais para responder.')));request.on('error',reject);if(body)request.write(body);request.end();});}
...
async function runPixCopyPaste(db,flow,lead,connection,node,variables){
  ...
  if(existing?.copia_e_cola){await sendText(connection,lead.phone,existing.copia_e_cola);return {lead,variables};}
  ...
  const flowData=setAt(context.flow_data,saveTo,{copia_e_cola:code,txid:charge.data.txid||txid,valor:payload.valor.original,expira_em_segundos:expiration,criado_em:new Date().toISOString()});const {data:updated,error}=await db.from('leads').update({order_context:{...context,flow_data:flowData,flow_execution:null},status:'in_progress',updated_at:new Date().toISOString()}).eq('id',lead.id).eq('owner_id',lead.owner_id).eq('connection_id',connection.id).select().single();if(error)throw error;
  await sendText(connection,updated.phone,code);return {lead:updated,variables:variablesFor(updated)};
}
...
export async function executeFlow({db,flow,lead,connection,resumeAfterId=null,resumeHandle=null,audios=[]}){
  ...
  for(let steps=0;node&&steps<50;steps+=1){const kind=node.data?.kind;const config=node.data?.config||{};
    ...
    if(kind==='pixCopyPaste'){const result=await runPixCopyPaste(db,flow,currentLead,connection,node,variables);currentLead=result.lead;variables={...result.variables,kie:{...(result.variables.kie||{}),audios:readyAudios}};}
