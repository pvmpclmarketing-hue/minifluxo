...
export async function POST(request,{params}){
  try{
    ...
    const body=await request.json().catch(()=>({}));const result=await retryKieDelivery({db,flow,lead,connection,assumeFirstTrackDelivered:body.assume_first_track_delivered===true});return NextResponse.json({received:true,result,status:result.completed?'completed':'delivering'});
  }catch(error){console.error('[retry delivery] failed',{lead_id:params?.id||null,error:error?.message||String(error)});return NextResponse.json({error:error.message||'Não foi possível reenviar a música.'},{status:500});}
}
