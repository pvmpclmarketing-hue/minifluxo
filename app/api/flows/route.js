...
export async function POST(request) {
  try {
    ...
    const { data, error } = await adminClient().from('flows').insert({ owner_id:user.id, name:body.name.trim(), description:body.description?.trim() || null, share_code }).select().single();
