...
import { configureGlobalUazWebhook, createUazInstance } from '../uazapi';
...
export async function POST(request) {
  try {
    ...
    if(body.provider==='uazapi'){const instance=await createUazInstance(body.instance_name||body.name);payload.instance_name=instance.instanceName;payload.uazapi_token_cipher=encryptSecret(instance.token);payload.uazapi_token_hash=hashSecret(instance.token);await configureGlobalUazWebhook();}
