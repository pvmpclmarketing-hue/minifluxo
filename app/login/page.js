...
export default function Login() {
  ...
  return <main className="auth-page"><section className="brand-panel"><div className="mark">W</div><p className="eyebrow">AUTOMAÇÃO DE ENTREGAS</p><h1>Whats<span>Entregável</span></h1><p>Do pedido à música pronta. Tudo no WhatsApp.</p><div className="auth-flow"><b>Pedido</b><i>→</i><b>Pix</b><i>→</i><b>IA</b><i>→</i><b>Música</b></div></section><section className="login-card"><p className="eyebrow">ACESSO SEGURO</p><h2>Entre no painel</h2><p>Use o e-mail cadastrado no WhatsEntregavel.</p><form onSubmit={submit}><label>E-mail<input type="email" value={email} onChange={e=>setEmail(e.target.value)} required /></label><label>Senha<input type="password" value={password} onChange={e=>setPassword(e.target.value)} required /></label>{error && <p className="form-error">{error}</p>}<button className="login-submit" disabled={loading}><span>{loading?'...':'→'}</span>{loading ? 'Entrando...' : 'Entrar no painel'}</button></form></section></main>;
}
