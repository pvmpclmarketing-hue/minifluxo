...
export function createTimeline(input: {audioUrl:string; photos:string[]; lyrics:string; lyricsTimestamps?:TimedCaption[]|null; introText?:string|null; duration?:number; backgroundUrl?:string|null}): VideoTimeline {
  const duration = Math.max(1, input.duration ?? 60);
  if (!input.backgroundUrl && !input.photos.length) throw new Error('Envie um fundo ou fotos para o vídeo.');
  return { width:1080, height:1920, fps:30, duration, audio:{url:input.audioUrl,start:0,volume:1}, photos:input.photos.length ? buildPhotoTimeline(input.photos,duration) : [], backgroundUrl:input.backgroundUrl || null, lyrics:input.lyricsTimestamps?.length ? input.lyricsTimestamps : splitLyrics(input.lyrics,duration), introText:input.introText, style:{captionAnimation:input.backgroundUrl?'lyric_build':'instagram'} };
}
