...
async function alignLyrics(originalLyrics, words, apiKey) {
  const prompt = `Você sincroniza letras de músicas em português para animação editorial. Compare a LETRA ORIGINAL com a TRANSCRIÇÃO COM TEMPOS. Retorne APENAS JSON válido no formato {"blocks":[{"start":0,"end":0,"text":"","units":[{"text":"","start":0,"end":0}]}]}.\n\nRegras obrigatórias:\n- Remova marcadores como [Verso 1], [Refrão] e [Ponte].\n- O texto de cada bloco deve usar a letra original corrigida, nunca títulos de seção.\n- Agrupe 3 a 8 palavras por bloco, mantendo uma leitura natural.\n- Use o início da primeira palavra e o fim da última palavra correspondente na transcrição; não invente tempos.\n- Respeite a ordem cantada, não pule trechos e não crie novas palavras.\n- Use apenas blocos que terminem até 60 segundos.\n- Para cada bloco, devolva 2 a 4 units visuais, em ordem. Uma unit tem 1 palavra importante ou 2 a 4 palavras curtas e será desenhada em uma linha separada.\n- Cada unit deve ter o início da primeira palavra e o fim da última palavra dela na transcrição. Não invente tempos nem junte palavras não consecutivas.\n\nLETRA ORIGINAL:\n${lyricsForCaptions(originalLyrics)}\n\nTRANSCRIÇÃO COM TEMPOS:\n${JSON.stringify(words)}`;
