...
export function blocksFromTranscript(words) {
  ...
  })).filter(item => item.text && Number.isFinite(item.start) && Number.isFinite(item.end) && item.start >= 0 && item.start < 3600 && item.end >= item.start).sort((a, b) => a.start - b.start);
  const timed = repairOpeningTiming(recognized);
  const result = timed.map((word, index) => ({
    ...word,
    end: Math.max(word.end, word.start + 0.04, timed[index + 1]?.start > word.start ? timed[index + 1].start : word.end + .7),
  }));
  if (!result.length) throw new Error('A transcrição não retornou palavras para a música.');
