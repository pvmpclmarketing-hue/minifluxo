...
export function normalizeCaptionBlocks(value) {
  ...
  return value.map(item => ({
    ...
    text: clean(item?.text).toUpperCase(),
  })).filter(item => item.text && Number.isFinite(item.start) && Number.isFinite(item.end) && item.start >= 0 && item.end > item.start && item.end <= 60);
}

export function groupLyricsIntoCaptionBlocks(lyrics) {
  const words = clean(lyrics).split(' ').filter(Boolean);
  const groups = [];
  for (let index = 0; index < words.length; index += 6) groups.push(words.slice(index, index + 6).join(' ').toUpperCase());
