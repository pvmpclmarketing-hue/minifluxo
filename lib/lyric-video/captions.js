...
function normalizeUnits(value, blockStart, blockEnd) {
  if (!Array.isArray(value)) return [];
  return value.map(item => ({
    text: clean(item?.text),
    start: Number(item?.start),
    end: Math.min(Number(item?.end), blockEnd),
  })).filter(item => item.text && Number.isFinite(item.start) && Number.isFinite(item.end) && item.start >= blockStart && item.end > item.start && item.end <= blockEnd).sort((a, b) => a.start - b.start);
}
...
export function normalizeCaptionBlocks(value) {
  ...
  return blocks.map((item, index) => {
    const end = Math.min(item.end, (blocks[index + 1]?.start ?? item.end) - 0.08);
    return { start: item.start, end, text: item.text, units: normalizeUnits(item.units, item.start, end) };
