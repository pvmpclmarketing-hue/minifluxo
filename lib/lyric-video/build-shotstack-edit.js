...
// Short, staggered clips create the blinking/pulsing hearts familiar from
// short-form editors, while keeping the central safe area free for the lyric.
function heartEffectTracks(themeKey) {
  const beats = [
    { x: -0.37, y: -0.3, delay: 1.2, size: 70 },
    { x: 0.37, y: 0.26, delay: 3.1, size: 54 },
    { x: -0.34, y: 0.37, delay: 5, size: 46 },
  ];
  return beats.map(({ x, y, delay, size }) => ({ clips: Array.from({ length: 10 }, (_, beat) => ({
    asset: {
      type: 'rich-text', text: '♥',
      font: { family: 'Inter', size, weight: 900, color: HEART_COLORS[themeKey], opacity: 0.68 },
      stroke: { width: 1, color: '#7A2438', opacity: 0.28 },
      shadow: { offsetX: 0, offsetY: 2, blur: 7, color: '#FEE8EE', opacity: 0.55 },
      align: { horizontal: 'center', vertical: 'middle' },
      animation: { preset: 'fadeIn', duration: 0.16 },
    },
    start: round(delay + beat * 6), length: 0.72, width: 150, height: 150,
