...
test('long phrases are split without loss, fit the central safe region and retain previous lines', () => {
  ...
  const clips = edit.timeline.tracks.flatMap(t => t.clips).filter(c => c.asset.type === 'rich-text' && c.asset.text !== '♥' && c.start >= 23);
  ...
  for (const clip of clips) {
    ...
    assert.deepEqual(clip.asset.stroke, { width: 2, color: '#120A0D', opacity: 0.9 });
    assert.deepEqual(clip.asset.animation, { preset: 'ascend', duration: 0.28, direction: 'up' });
  ...
  assert.equal(edit.timeline.tracks.at(-1).clips[0].effect, 'zoomInSlow');
  const hearts = edit.timeline.tracks.flatMap(track => track.clips).filter(clip => clip.asset.text === '♥');
  assert.equal(hearts.length, 192);
  assert.ok(hearts.every(heart => heart.length === 0.64 && heart.asset.font.opacity === 0.94));
  assert.ok(hearts.every(heart => Math.abs(heart.offset.x) > 0.3));
});
...
test('transcript moves a hallucinated opening prompt fragment to the first voiced cluster', () => {
  const blocks = blocksFromTranscript([
    { word: 'Foi', start: 0, end: 0.35 },
    { word: 'na', start: 0.35, end: 0.62 },
    { word: 'faculdade', start: 12.88, end: 13.5 },
  ]);
  const words = blocks.flatMap(block => block.units);
  assert.ok(words[0].start > 12);
  assert.equal(words[0].end, words[1].start);
  assert.ok(Math.abs(words[1].end - words[2].start) < 0.001);
});

test('transcript repairs an opening word stretched across the instrumental intro', () => {
  const blocks = blocksFromTranscript([
    { word: 'Foi', start: 0, end: 0.84 },
    { word: 'na', start: 0.84, end: 12.88 },
    { word: 'empresa', start: 12.88, end: 13.48 },
  ]);
  const words = blocks.flatMap(block => block.units);
  assert.ok(words[0].start > 11);
  assert.ok(Math.abs(words[1].end - words[2].start) < 0.001);
});
